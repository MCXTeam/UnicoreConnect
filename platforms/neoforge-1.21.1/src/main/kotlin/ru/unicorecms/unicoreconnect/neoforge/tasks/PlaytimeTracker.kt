package ru.unicorecms.unicoreconnect.neoforge.tasks
...
class PlaytimeTracker(private val platform: NeoPlatformImpl) {
    private val lastActivity = ConcurrentHashMap<UUID, Long>()
    private var task: Cancellable? = null

    fun start() {
        task = platform.neoScheduler.repeatSync(TICKS_PER_MINUTE, TICKS_PER_MINUTE) { tick() }
    }
    ...
    private fun moved(uuid: UUID): Boolean {
        val handle = (platform.player(uuid) as? ru.unicorecms.unicoreconnect.neoforge.NeoPlayer)?.handle() ?: return false
