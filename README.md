...
Выдача привилегий и покупок может доходить до сервера двумя путями. В карточке сервера в панели
CMS (**Серверы → Выдача**) выбирается способ: **RCON** — сайт сам подключается к серверу и
выполняет команды, **Плагин (UnicoreConnect)** — команды складываются в очередь, а забирает и
выполняет их плагин.

Во втором режиме порт RCON открывать не нужно, и команды не теряются, пока сервер лежит: они
дождутся его в очереди. Плагин забирает их, когда сайт присылает событие `run_commands`, и
дополнительно раз в минуту — на случай, если вебсокет был оборван. Выполненные и упавшие команды
подтверждаются сайту, неподтверждённые сайт через пять минут вернёт в очередь сам.

Отдельной настройки в конфиге плагина для этого нет: режим выбирается на стороне сайта, плагин
просто обслуживает свою очередь.
...
1. [Создайте API-ключ](https://unicorecms.ru/docs/api/keys) с правом `kernel.connect`, в разрешённых адресах укажите IP игрового сервера.
